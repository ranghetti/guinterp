...
#' @param indata TODO
#' @param metric one between f_rangev, f_rangey, f_zscorey, f_rbiasy, f_rangeq, f_pos
...
filter_pts <- function(
  ...
  if (any(
    ...
    !all(c("uid","sid","lat","lon","idfield","selvar","f_rangev","f_rangey","f_zscorey","f_rbiasy","f_rangeq","f_pos","filter") %in% names(indata))
  )) {
  ...
  # Apply filter
  if (metric == "rangev") {
    outdata[sid <= samplesize & (speed < value[1] | speed > value[2]), f_rangev := TRUE]
    outdata[sid <= samplesize & !(speed < value[1] | speed > value[2]), f_rangev := FALSE]

  } else if (metric == "rangey") {
    outdata[sid <= samplesize &  (selvar < value[1] | selvar > value[2]), f_rangey := TRUE]
    outdata[sid <= samplesize & !(selvar < value[1] | selvar > value[2]), f_rangey := FALSE]
    ...
    if (byfield) {
      ...
      for (sel_field in unique(outdata$idfield)) {
        ...
        outdata[idfield == sel_field & sid <= samplesize & ((selvar - outdata_avg)/outdata_sd < value[1] | (selvar - outdata_avg)/outdata_sd > value[2]), f_zscorey := TRUE]
        outdata[idfield == sel_field & sid <= samplesize & !((selvar - outdata_avg)/outdata_sd < value[1] | (selvar - outdata_avg)/outdata_sd > value[2]), f_zscorey := FALSE]
      }
    } else {
      outdata_avg <- outdata[sid <= samplesize, mean(selvar,na.rm = TRUE)]
      outdata_sd <- outdata[sid <= samplesize, sd(selvar,na.rm = TRUE)]
      outdata[sid <= samplesize & ((selvar - outdata_avg)/outdata_sd < value[1] | (selvar - outdata_avg)/outdata_sd > value[2]), f_zscorey := TRUE]
      outdata[sid <= samplesize & !((selvar - outdata_avg)/outdata_sd < value[1] | (selvar - outdata_avg)/outdata_sd > value[2]), f_zscorey := FALSE]
    }
    ...
    if (byfield) {
      ...
      for (sel_field in unique(outdata$idfield)) {
        outdata_avg <- outdata[idfield == sel_field & sid <= samplesize, mean(selvar,na.rm = TRUE)]
        outdata[idfield == sel_field & sid <= samplesize & ((selvar - outdata_avg)/outdata_avg < value[1] | (selvar - outdata_avg)/outdata_avg > value[2]), f_rbiasy := TRUE]
        outdata[idfield == sel_field & sid <= samplesize & !((selvar - outdata_avg)/outdata_avg < value[1] | (selvar - outdata_avg)/outdata_avg > value[2]), f_rbiasy := FALSE]
      }
    } else {
      outdata_avg <- outdata[sid <= samplesize, mean(selvar,na.rm = TRUE)]
      outdata[sid <= samplesize & ((selvar - outdata_avg)/outdata_avg < value[1] | (selvar - outdata_avg)/outdata_avg > value[2]), f_rbiasy := TRUE]
      outdata[sid <= samplesize & !((selvar - outdata_avg)/outdata_avg < value[1] | (selvar - outdata_avg)/outdata_avg > value[2]), f_rbiasy := FALSE]
    }
    ...
    if (byfield) {
      ...
      for (sel_field in unique(outdata$idfield)) {
        ...
        outdata[idfield == sel_field & sid <= samplesize & (selvar < outdata_rangeq[1] | selvar > outdata_rangeq[2]), f_rangeq := TRUE]
        outdata[idfield == sel_field & sid <= samplesize & !(selvar < outdata_rangeq[1] | selvar > outdata_rangeq[2]), f_rangeq := FALSE]
      }
    } else {
      outdata_rangeq <- outdata[sid <= samplesize, quantile(selvar,value,na.rm = TRUE)]
      outdata[sid <= samplesize & (selvar < outdata_rangeq[1] | selvar > outdata_rangeq[2]), f_rangeq := TRUE]
      outdata[sid <= samplesize & !(selvar < outdata_rangeq[1] | selvar > outdata_rangeq[2]), f_rangeq := FALSE]
    }
    ...
    if (byfield) {
      ...
      for (sel_field in unique(outdata$idfield)) {
        ...
        if (nrow(sel_inlayer) > 0) {
          ...
          outdata[idfield == sel_field & sid <= samplesize, f_pos := TRUE]
        }
      }
    } else {
      outdata_pos <- !colSums(suppressMessages(
  ...
  outdata[sid <= samplesize, filter := f_rangev|f_rangey|f_zscorey|f_rbiasy|f_rangeq|f_pos]
...
filter_pts_reset <- function(indata, filters = NA) {
  ...
  if (is.na(filters)) { # if NA, reset all; otherwise, only specified filters
    outdata[,c("f_rangev","f_rangey","f_zscorey","f_rbiasy","f_rangeq","f_pos","filter") := as.list(rep(FALSE,7))]
  } else {
    # TODO check that "filters" contains only allowed values
    outdata[,paste0("f_",filters) := as.list(FALSE)]
    outdata[,filter := f_rangev|f_rangey|f_zscorey|f_rbiasy|f_rangeq|f_pos]
  }
  return(outdata)
}
