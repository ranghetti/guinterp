...
observeEvent(rv$change_interp, {
  output$hist <- renderPlot({
    ...
    rv$hist_breaks <- hist(plot = FALSE, rv$inputpts_points[sid3<=input$samplesize_view & selvar>=rv$hist_range[1] & selvar<=rv$hist_range[2],]$selvar, breaks=50)$breaks
    rv$hist_ylim   <- c(0,max(hist(plot = FALSE, rv$inputpts_points[sid3<=input$samplesize_view & selvar>=rv$hist_range[1] & selvar<=rv$hist_range[2],]$selvar, breaks=50)$counts)) /
      nrow(rv$inputpts_points[sid3<=input$samplesize_view & selvar>=rv$hist_range[1] & selvar<=rv$hist_range[2],])

    p <- ggplot2::ggplot(rv$inputpts_points, ggplot2::aes(x=selvar)) +
      ggplot2::geom_histogram(data=rv$inputpts_points[sid3<=input$samplesize_view,], colour="white", aes(y = stat(count) / sum(count), fill=filter), breaks=rv$hist_breaks) +
