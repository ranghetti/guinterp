  ...
  switch(
    ...
    en = shiny::showModal(shiny::modalDialog(
      ...
      shiny::p(shiny::HTML(
        ...
        "if this value is lower than 500).</li>",
        "<li><strong>Subset points to interpolate</strong>:",
        "reduce the number of points used during interpolation",
        "(in the face of a faster interpolation, result will be less precise).",
        "Note: using this option in case points are not intra-polygon evenly",
        "distributed could lead to bad results.</li>",
        "<li><strong>Sampling scheme within polygons</strong>",
        "(visible only if multiple polygons are used):",
        "it allows changing the method used to subsample points,",
        "so reducing problems in case of points not evenly distributed among",
        "polygons. Options are:<ul>",
        "<li><em>Random</em>: completely-random selection (suitable in the case",
        "of evenly distributed points);</li>",
        "<li><em>Stratified (fixed)</em>: the amount of points used for each polygon",
        "is the same (until all points within less represented polygons have been selected);",
        "it is suitable in case of polygons sampled with different methods;</li>",
        "<li><em>Stratified (area-weighted)</em>: similar to the previous one,",
        "this method reduces the numer of points in smaller polygons, so being",
        "suitable in case of polygons characterised by etherogeneous areas",
        "(D'Hondt method is used, using areas as weights);</li>",
        "<li><em>Proportional</em>: proportion of points selected within",
        "each polygon is the same (using large numbers, this method tends to",
        "the random selection).</li></ul></li></ul>"
      )),
      easyClose = TRUE,
      footer = NULL
    )),
    ...
    it = shiny::showModal(shiny::modalDialog(
      ...
      shiny::p(shiny::HTML(
        ...
        "totale di punti nel caso questo sia minore di 500).</li>",
        "<li><strong>Usa un campione di punti per l'interpolazione</strong>:",
        "riduci il numero di punti usato per l'interpolazione",
        "(a fronte di una più veloce esecuzione, il risultato sarà più impreciso).",
        "Nota: usare questa opzione con punti non omogeneamente distributiti entro",
        "ogni poligono potrebbe ridurre di molto la qualità del risultato.</li>",
        "<li><strong>Schema di campionamento nei poligoni</strong>",
        "(visibile solo nel caso di utilizzo di poligoni multipli):",
        "permette di modificare il metodo con cui i punti vengono sottocampionati,",
        "riducendo i problemi nel caso di punti non omogeneamente",
        "distribuiti tra i diversi poligoni. Le opzioni sono:<ul>",
        "<li><em>Casuale</em>: selezione completamente casuale (adatta nel caso",
        "di punti omogeneamente distribuiti);</li>",
        "<li><em>Stratificato (fisso)</em>: il numero di punti per ogni poligono",
        "è lo stesso (fino alla saturazione di punti nei poligoni meno rappresentati);",
        "è adatto nel caso di poligoni campionati in modo diverso;</li>",
        "<li><em>Stratificato (pesato sulle aree)</em>: simile al precedente,",
        "ma che riduce il numero di punti nei poligoni più piccoli, e quindi adatto",
        "nel caso di poligoni con superfici molto eterogeneee (viene usato",
        "il metodo D'Hondt usando le aree dei poligoni come pesi);</li>",
        "<li><em>Proporzionale</em>: la proporzione di punti selezionati entro",
        "ogni poligono è la stessa (con grandi numeri, il metodo tende alla",
        "selezione casuale).</li></ul></li></ul>"
      )),
      easyClose = TRUE,
      footer = NULL
    ))
